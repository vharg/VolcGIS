...
buffer = pd.read_csv('csv/MASTER_buffer.csv', index_col=None)
...
for iRow in range(0, len(expName)):
    ...
    for iCol in range(0, len(regionsRatio)):
        ...
        for iV in [5,4,3]:
            ...
            axes[iRow][iCol].axhline(y=dataCol.loc[iV,50][expCol[iRow]].mean(), color=cmapi[vCount], linestyle=':')
            if plotCI:
...
for iRow in range(0, len(expName)):
    ...
    for iCol in range(0, len(regionsRatio)):
        ...
        for iV in [100,30,10]:
            ...
            axes[iCol].axhline(y=dataCol.loc[iV][expCol[iRow]].mean(), color=cmapi[vCount], linestyle=':')
            vCount += 1
